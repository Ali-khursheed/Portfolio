		...
		<section class="mt-8 relative">
			<div class="grid grid-cols-1">
				...
				<div
					...
					<p
						class="flex justify-start text-xl dark:text-gray-200 font-semibold">
					...
					<div class="flex flex-col sm:flex-row sm:items-center gap-4">
						<btn
							...
							<icon [size]="20" [path]="fileIcon" style="color: white;"></icon>
							<span>Resume</span>
						</btn>
						<social-link />
					</div>
...
export class Intro {
	...
	updateText(): void {
		this.currentGreetingIndex.set(
			(this.currentGreetingIndex() + 1) % this.profile().greetings.length
		);
		this.changingText.set(
			this.profile().greetings[this.currentGreetingIndex()]
		);
	}
}
